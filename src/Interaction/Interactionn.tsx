import store, { setRelation } from "../Redux/store";
...
class Interaction {
	...
	validateNode(droppingNode: any, options: any, graph: any) {
		this.graph = graph;
	...
	addServiceNode(oldNode: any, title: string) {
		...
		if(oldNode.id !== 'empty') {
			...
			store.dispatch(setRelation(false));
		}
	...
	addParallelNode(oldNode: any) {
		// open relation setting panel
		store.dispatch(setRelation(true));
		...
		let parallelNodes: Array<any> = [];
		let parallelUpEdges: Array<any>= [];
		let parallelDownEdges: Array<any> = [];
		...
		this.graph.on("AddParallelService", (serviceNode: any) => {
	...
	deleteServiceNode(serviceNode: any) {
	...
	deleteParalleleNode(startNode: any, finishNode: any) {
		// reset routeNum
